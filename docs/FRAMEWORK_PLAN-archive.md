# Framework Plan — Archive

Closed milestones and completed-pass logs moved out of `docs/FRAMEWORK_PLAN.md` so the live plan
stays small enough to read on every task. Nothing here is a rule or a pending item — it is history.
Rotation rule: see `docs/FRAMEWORK_PLAN.md` (Update Protocol).

Rotated on 2026-08-01.
...
- Continued builder-control parity with paired `DualTransitionBuilder` + `RepeatingAnimationBuilder<T>` ports,
  including source-shaped nested enter/exit transitions, interrupted-direction continuity, restart/reverse looping,
  pause/resume behavior, curve/duration updates, stable-child reuse, focused tests, and mirrored C#/Dart probes.
...
- Continued paired core transform coverage with `FractionalTranslation` + `RotatedBox`, including source-shaped
  fractional paint/hit-test offsets, layout-time quarter-turn constraint and size transposition, transformed paint/
  semantics geometry, negative turn support, focused tests, and mirrored C#/Dart probes.
...
- Continued paired core utility-widget coverage with `StatefulBuilder` + `LookupBoundary`, including local
  state-setter rebuild ownership, exact-type inherited dependency and non-dependent reads, bounded widget/state/
  render-object ancestor queries, boundary-aware element visitors, focused tests, and a mirrored C#/Dart probe.
...
- Continued paired core notification coverage with `NotificationListener<T>` + `ScrollNotificationObserver`,
...
- Continued paired core backdrop-filter coverage with `BackdropFilter` + `BackdropGroup`, including direct,
  bounded-blur, and composed filter configurations, inherited/manual group keys, bitmap-backed scene-prefix
  sampling, grouped input reuse, retained layers, blend and enabled policies, focused tests, and mirrored C#/Dart
  image probes.
...
- Replaced the earlier unconstrained layout baseline with paired `ConstraintsTransformBox` + `UnconstrainedBox`
  source parity: all standard constraint transforms, arbitrary normalized transform delegates, physical/directional
  alignment, overflow-only clip policy, debug overflow markers, and Flutter's thin `UnconstrainedBox` composition
  over the shared render primitive. `LimitedBox` retains max clamps only on parent-unbounded axes.
- Expanded the mirrored C#/Dart unconstrained-layout demo with a direct max-width transform and runtime clip toggle.
...
## M4 Material Library Rewrite — completed-pass log

Per-control completion notes accumulated while M4 was running. The live record of what shipped is
`CHANGELOG.md`; the live record of what is left is `docs/MATERIAL_TODO.md`.

Kickoff note (2026-03-12):

- Replaced the initial flat Material theme surface with Flutter-shaped `ColorScheme`, `TextTheme`, and `Typography`:
...
- Closed `IconButton` against current `icon_button.dart` + `icon_button_theme.dart`: all standard/filled/tonal/
  outlined constructors expose the source properties; M2 uses legacy constraints/padding/icon-theme/`InkResponse`
...
- Added paired core `ModalBarrier` + `AnimatedModalBarrier` parity with source-shaped dismiss/pop behavior,
  any-button opaque hit-test capture, platform-aware semantics, notifier-driven semantics clipping, system-alert
  dispatch, animated colors, and sibling-semantics blocking; Material dialog routes now reuse the shared animated
  barrier.
...
- Expanded `MaterialScaffoldTests` with focused RTL drawer edge coverage: start/end drawer edge-drag open behavior now has explicit RTL regression coverage (start opens from right edge, end opens from left edge), including `MediaQuery.padding` edge-width extension checks on both sides in RTL.
- Expanded `MaterialScaffoldTests` with focused drawer interaction-gating coverage: scrim tap now has explicit close-vs-lock coverage for `drawerBarrierDismissible` true/false paths, edge-open drag disable flags are now covered (`drawerEnableOpenDragGesture` and `endDrawerEnableOpenDragGesture`), and desktop-platform gesture gating is now covered (`TargetPlatform.Windows` edge drags do not open drawers).
- Expanded `MaterialScaffoldTests` with alternating drawer-drag stress coverage: rapid start->end and end->start drag choreography is now explicitly covered (edge-open + panel-close sequences on both sides), with mutual-exclusion and post-settle visibility assertions to guard against start/end state interop regressions.
...
- Added Material `LinearProgressIndicator` baseline in `Plumix.Material`:
  - introduced framework `LinearProgressIndicator` in `src/Plumix.Material/ProgressIndicator.cs` with determinate (`value`) and indeterminate (`value: null`) modes, Flutter-like indeterminate two-segment animation timing, mode-aware defaults (M3: `primary` on `secondaryContainer`; M2: `primary` on `canvas`), widget/theme/default precedence, and RTL-aware fill direction.
  - expanded linear-progress parity with M3 stop-indicator + track-gap support: `LinearProgressIndicator` now exposes `stopIndicatorColor`, `stopIndicatorRadius`, and `trackGap` with precedence `widget -> ProgressIndicatorTheme -> mode defaults`; when `year2023=false` in M3, defaults match Flutter (`stopIndicatorColor=primary`, `stopIndicatorRadius=2`, `trackGap=4`), while `year2023=true` and M2 keep stop/gap disabled.
  - expanded linear-progress year2023 parity: `LinearProgressIndicator` now supports `year2023` (`widget -> ProgressIndicatorTheme.year2023 -> default true` in M3), with Flutter-like 2023/2024 default switching for stop-indicator visibility, track-gap visibility, and default border-radius behavior.
  - added progress-indicator theming surface (`ProgressIndicatorThemeData`, inherited `ProgressIndicatorTheme`, and `ThemeData.ProgressIndicatorTheme`) with linear color/track/min-height/border-radius overrides plus shared animation controller override (`controller`).
  - expanded linear-progress controller parity: `LinearProgressIndicator` now accepts `controller` and resolves animation source by precedence `widget.controller -> ProgressIndicatorTheme.controller -> internal controller`; value+controller conflicts now throw, and internal controller ticking is gated to the internal-source path only.
  - expanded linear-progress `valueColor` parity: `LinearProgressIndicator` now exposes `valueColor` via `IValueListenable<Color?>` (including `ValueNotifier<Color?>` and `AlwaysStoppedAnimation<Color?>`) with Flutter-like precedence `valueColor -> color -> ProgressIndicatorTheme.color -> theme primary`, including live listenable-driven updates and null-value fallback to the remaining precedence chain.
  - added focused `MaterialLinearProgressIndicatorTests` coverage in `src/Plumix.Tests/MaterialLinearProgressIndicatorTests.cs` for constructor guards, M2/M3 defaults, year2023 2023/2024 defaults, stop-indicator/track-gap defaults, theme/widget precedence (including `stopIndicator*` + `trackGap` + `year2023`), value clamping, explicit/theme controller usage for indeterminate animation, `valueColor` precedence + runtime listenable updates, RTL direction wiring, and semantics-label percentage fallback.
  - added C#/Dart sample parity demo route/page for runtime verification (`LinearProgressIndicator` route in Material tab with determinate/indeterminate, M2/M3, `year2023` toggle, theme/widget overrides, `valueColor` toggle, and RTL probes).
  - closed previous value-color API divergence: progress indicators now accept listenable value sources beyond `ValueNotifier` (including `AlwaysStoppedAnimation`) to mirror Flutter usage patterns.
- Added Material `CircularProgressIndicator` baseline in `Plumix.Material`:
  ...
  - expanded progress-indicator theming surface (`ProgressIndicatorThemeData`) with circular baseline fields (`circularTrackColor`, `circularStrokeWidth`, `circularStrokeAlign`, `circularConstraints`, `circularSize`, `circularStrokeCap`) and reused inherited `ProgressIndicatorTheme` + `ThemeData.ProgressIndicatorTheme` precedence.
  - expanded circular-progress parity with M3 track-gap support: `CircularProgressIndicator` now exposes `trackGap` with precedence `widget -> ProgressIndicatorTheme -> mode defaults` (`M3` default `4`; `M2` disabled/null), and determinate track paint now renders a Flutter-like gap between active arc and background track.
  - expanded circular-progress parity with `strokeCap` support: `CircularProgressIndicator` now exposes `strokeCap` and resolves it by precedence `widget -> ProgressIndicatorTheme.CircularStrokeCap -> default-null`, while render paint applies Flutter-like null-cap defaults (determinate `butt`, indeterminate `square`) and uses explicit `strokeCap` for both foreground and track stroke caps.
  - expanded circular-progress parity with `strokeAlign` support: `CircularProgressIndicator` now exposes `strokeAlign`, resolves precedence as `widget -> ProgressIndicatorTheme.CircularStrokeAlign -> default(0.0)`, and uses Flutter-like signed stroke offset geometry for inside/center/outside arc painting.
  - expanded circular-progress parity with Flutter-like constraints precedence: `CircularProgressIndicator` now resolves layout constraints as `widget.constraints -> widget.size (legacy fallback) -> theme.circularConstraints -> theme.circularSize (legacy fallback) -> mode defaults` (`M3`: `40x40`, `M2`: `36x36`) and composes through `ConstrainedBox`.
  - expanded circular-progress year2023 parity: `CircularProgressIndicator` now supports `year2023` (`widget -> ProgressIndicatorTheme.year2023 -> default true` in M3), with Flutter-like mode switching between 2023 and 2024 defaults (track visibility, `trackGap` enablement, default constraints, default stroke alignment, and implicit indicator line-cap behavior).
  - expanded circular-progress controller parity: `CircularProgressIndicator` now accepts `controller` and resolves animation source by precedence `widget.controller -> ProgressIndicatorTheme.controller -> internal controller`; value+controller conflicts now throw, and internal controller ticking is gated to the internal-source path only.
  - expanded circular-progress `valueColor` parity: `CircularProgressIndicator` now exposes `valueColor` via `IValueListenable<Color?>` (including `ValueNotifier<Color?>` and `AlwaysStoppedAnimation<Color?>`) with Flutter-like precedence `valueColor -> color -> ProgressIndicatorTheme.color -> theme primary`, including live listenable-driven updates and null-value fallback.
  - closed adaptive-platform parity for circular progress: `CircularProgressIndicator.Adaptive(...)` now routes iOS/macOS to `CupertinoActivityIndicator` (including determinate `partiallyRevealed` progress mapping) and keeps the existing Material path for non-Apple targets.
  - closed adaptive semantics parity for circular progress: iOS/macOS adaptive Cupertino branch now preserves `Semantics` wrapping (including computed percent fallback when `semanticsValue` is absent) instead of bypassing semantics on early return.
  - added focused `MaterialCircularProgressIndicatorTests` coverage in `src/Plumix.Tests/MaterialCircularProgressIndicatorTests.cs` for constructor guards, M2/M3 defaults, theme/widget precedence (including `trackGap`, `strokeCap`, `strokeAlign`, `constraints`, and `year2023`), value clamping, explicit/theme controller usage for indeterminate animation, `valueColor` precedence + runtime listenable updates, semantics-label percentage fallback, and retained legacy `size` fallback behavior.
...
- Added Material `Slider` baseline in `Plumix.Material`:
  ...
  - expanded slider parity with Flutter-like secondary-track support: `Slider` now exposes `secondaryTrackValue` + `secondaryActiveColor`, validates secondary value range/finite constraints, resolves secondary-track colors by precedence (`widget -> SliderTheme.secondaryActiveTrackColor -> default primary@0.54`) with disabled fallback (`SliderTheme.disabledSecondaryActiveTrackColor -> onSurface@0.12`), and paints secondary track only when the secondary value is ahead of the active thumb (`secondaryTrackValue > value`) for both LTR/RTL.
