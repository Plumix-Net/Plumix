...
- Added Material `LinearProgressIndicator` baseline in `Plumix.Material`:
  - introduced framework `LinearProgressIndicator` in `src/Plumix.Material/ProgressIndicator.cs` with determinate (`value`) and indeterminate (`value: null`) modes, Flutter-like indeterminate two-segment animation timing, mode-aware defaults (M3: `primary` on `secondaryContainer`, rounded radius from height; M2: `primary` on `canvas`, square radius), widget/theme/default precedence, and RTL-aware fill direction.
  - expanded linear-progress parity with M3 stop-indicator + track-gap support: `LinearProgressIndicator` now exposes `stopIndicatorColor`, `stopIndicatorRadius`, and `trackGap` with precedence `widget -> ProgressIndicatorTheme -> mode defaults` (`M3` defaults: primary stop indicator, radius `2`, track gap `4`; `M2`: disabled/null), and painter choreography now mirrors Flutter track-gap behavior for both determinate and indeterminate phases.
  ...
  - added focused `MaterialLinearProgressIndicatorTests` coverage in `src/Plumix.Tests/MaterialLinearProgressIndicatorTests.cs` for constructor guards, M2/M3 defaults, stop-indicator/track-gap defaults, theme/widget precedence (including `stopIndicator*` + `trackGap`), value clamping, explicit/theme controller usage for indeterminate animation, RTL direction wiring, and semantics-label percentage fallback.
  ...
  - remaining divergence: advanced Flutter API surface (`valueColor` and `year2023`) remains out of this baseline scope.
