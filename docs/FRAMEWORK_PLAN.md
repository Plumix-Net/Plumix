...
- Added Material `LinearProgressIndicator` baseline in `Plumix.Material`:
  ...
  - expanded linear-progress `valueColor` parity: `LinearProgressIndicator` now exposes `valueColor` (`ValueNotifier<Color?>`) with Flutter-like precedence `valueColor -> color -> ProgressIndicatorTheme.color -> theme primary`, including live notifier-driven updates and null-value fallback to the remaining precedence chain.
  - added focused `MaterialLinearProgressIndicatorTests` coverage in `src/Plumix.Tests/MaterialLinearProgressIndicatorTests.cs` for constructor guards, M2/M3 defaults, year2023 2023/2024 defaults, stop-indicator/track-gap defaults, theme/widget precedence (including `stopIndicator*` + `trackGap` + `year2023`), value clamping, explicit/theme controller usage for indeterminate animation, `valueColor` precedence + runtime notifier updates, RTL direction wiring, and semantics-label percentage fallback.
  ...
  - remaining divergence: full Flutter `Animation<Color?>` type parity is approximated with framework `ValueNotifier<Color?>` in the current scope.
- Added Material `CircularProgressIndicator` baseline in `Plumix.Material`:
  ...
  - expanded circular-progress `valueColor` parity: `CircularProgressIndicator` now exposes `valueColor` (`ValueNotifier<Color?>`) with Flutter-like precedence `valueColor -> color -> ProgressIndicatorTheme.color -> theme primary`, including live notifier-driven updates and null-value fallback.
  ...
  - remaining divergence: full Flutter `Animation<Color?>` type parity remains out of this baseline scope (current framework uses `ValueNotifier<Color?>`).
