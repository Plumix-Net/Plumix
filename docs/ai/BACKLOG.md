...
| `widgets/raw_menu_anchor.dart` | `onCloseRequested` fires on every `MenuController.close`; root anchor detaches scroll listener on dispose. | verify (`OnCloseRequested` exists) |
