...
| `GlobalMaterialLocalizations` is not ported, so a Material app has only `DefaultMaterialLocalizations` (US English). The machinery it needs now exists: the pinned CLDR snapshot in `src/Plumix/Foundation/Intl`, `GlobalWidgetsLocalizations`, and `scripts/generate_localizations.py`. | `src/Plumix.Material/MaterialLocalizations.cs` vs `flutter_localizations/lib/src/material_localizations.dart` + `l10n/generated_material_localizations.dart` | Add a Material section to `scripts/generate_localizations.py` (same class/switch transliteration), hand-write `GlobalMaterialLocalizations` over the intl subset (it needs the `Hm`/`jm`/`yMMMd` skeletons and `NumberFormat` percent/compact forms Cupertino does not use), and mirror `flutter_localizations/test/material/*`. |
| `CupertinoPicker.HandleChildTap` is `async void`: after `await AnimateToItem` the continuation reads `EffectiveController.SelectedItem`, which throws once the picker has been unmounted. The exception escapes as an unhandled task fault and fails whichever test happens to be running (~2 of 10 full suite runs, pre-existing). | `src/Plumix.Cupertino/CupertinoPicker.cs` (`CupertinoPickerState.HandleChildTap`) vs `cupertino_ui/lib/src/picker.dart` | Guard the continuation on `Mounted` (and on the controller still having positions) the way Dart's `_CupertinoPickerState._handleTap` is protected by the widget lifetime; add a regression test that taps a child and unmounts before the animation settles. |
