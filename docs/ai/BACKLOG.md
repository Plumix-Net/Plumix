...
| --- | --- | --- |
| `ScaleGestureRecognizer` (`gestures/scale.dart`) is not ported: no pinch/rotate recognizer exists, so the trackpad pan/zoom events the framework now routes have no multi-touch consumer, and `Material`'s `InteractiveViewer`-style controls cannot be built. | new `src/Plumix/Gestures/ScaleGestureRecognizer.cs` vs `gestures/scale.dart` | Port the recognizer 1:1 including `_PointerPanZoomData` (`trackpadScrollCausesScale`, `trackpadScrollToScaleFactor = Offset(0, -1/200)`), the pointer-count/focal/span aggregation and the `computeScaleSlop`/`computePanSlop`/1.05-ratio acceptance test; tests from `scale_test.dart`. |
