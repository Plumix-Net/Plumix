...
| `SemanticsOwner` has no incremental dirty pipeline: it rebuilds the node tree and re-indexes it, where Dart keeps `_dirtyNodes`/`_detachedNodes`, `SemanticsNode._markDirty`/`debugIsDirty`, and a `sendSemanticsUpdate` drain loop. Traversal grafting and the `mapEquals` custom-action dirty check both hang off that loop. | `src/Plumix/Rendering/Semantics.cs` (`SemanticsOwner`, `SemanticsNode.UpdateWith`) vs `semantics/semantics.dart` | Port `attach`/`detach` id bookkeeping, `_markDirty` and the `sendSemanticsUpdate` drain loop first; then `_isDifferentFromCurrentSemanticAnnotation`'s `mapEquals` clause, then `traversalParentIdentifier`/`traversalChildIdentifier` with `_computeTraversalTransform` and the hit-test orphan filter. |
...
| `semantics/semantics.dart` | Traversal grafting (`traversalParentIdentifier`/`traversalChildIdentifier`, `_traversalChildTransform`, orphan filtering in hit-test order) and the `mapEquals` custom-action dirty check both ride on the incremental dirty pipeline, which Plumix does not have: `SemanticsOwner` rebuilds the node tree and has no `_dirtyNodes`/`_markDirty`/`sendSemanticsUpdate` drain loop. Custom-action dispatch, `getSemanticsNode` and the merged-descendant walk shipped 2026-08-28. | open (needs the dirty pipeline first) |
