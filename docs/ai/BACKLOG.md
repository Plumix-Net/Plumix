...
| Every Material control except the four buttons still builds on the C#-only `MaterialButtonCore` (IconButton, FloatingActionButton, Chips, NavigationBar/Rail/Drawer, MenuAnchor, ToggleButtons, Slider, RangeSlider, Switch, Carousel, SearchAnchor, DropdownMenu, MaterialButton, BottomNavigationBar). Flutter has no such class: `_IconButtonM3 extends ButtonStyleButton` and the rest compose `Material` + `InkWell` directly. | `src/Plumix.Material/MaterialButtonCore.cs` vs `material_ui/lib/src/{icon_button,floating_action_button,chip,navigation_bar,...}.dart` | Port each control onto `ButtonStyleButton` (or `Material` + `InkWell`) as it is next touched; when the last consumer is gone delete `MaterialButtonCore`, `ButtonTapTargetPadding`/`RenderButtonTapTargetPadding` (superseded by `InputPadding`/`RenderInputPadding`) and `ButtonStyle.SplashColor`. |
| `RenderButtonTapTargetPadding` duplicates `RenderInputPadding` but diverges from Dart with no child (Dart returns `Size.zero` and `0.0` intrinsics; it returns `minSize`), throws on a non-finite `minSize`, has no `computeDryBaseline`, and hit-tests without `MatrixUtils.forceToPoint`. | `src/Plumix.Material/MaterialButtonCore.cs` vs `material_ui/lib/src/button_style_button.dart` (`_RenderInputPadding`) | Point `MaterialButtonCore` at `InputPadding` and delete the duplicate, once its consumers' tests are re-checked. |
...
| Two debug-only pieces of the `Hero` port are missing: Dart's `NavigatorState._updateHeroController` reports a `FlutterError` from a post-frame callback when one `HeroController` ends up owned by two navigators, and `Hero`/`HeroMode` implement `debugFillProperties` (`tag`, `mode`). | `src/Plumix/Widgets/Navigation.NavigatorState.cs` (`UpdateHeroController`), `src/Plumix/Widgets/Hero.cs` vs `flutter/.../widgets/navigator.dart`, `heroes.dart` | Port the ownership check once Plumix has an equivalent of `FlutterError.reportError`; add the two `DebugFillProperties` overrides. Neither affects runtime behavior. |
