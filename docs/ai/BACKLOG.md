...
| The Material `Switch` is the last control building on the C#-only `MaterialButtonCore`; Flutter builds `_MaterialSwitch` on `ToggleableStateMixin` + `_SwitchPainter` and paints the focus/hover/press reaction itself instead of taking an ink splash. | `src/Plumix.Material/Switch.cs` (`SwitchState`), `src/Plumix.Material/MaterialButtonCore.cs` vs `material_ui/lib/src/switch.dart` | Derive `SwitchState` from `ToggleableState` (`src/Plumix/Widgets/Toggleable.cs`), build through `BuildToggleableChild` with a radial-reaction overlay, wire `Switch.adaptive` to the ported `CupertinoSwitch`, then delete `MaterialButtonCore`. |
