...
| --- | --- | --- |
| Trackpad pan/zoom is not modelled: core has no `PointerPanZoom{Start,Update,End}Event`, so `GestureRecognizer.addPointerPanZoom`/`addAllowedPointerPanZoom`/`isPointerPanZoomAllowed`, the pan/zoom branches of `DragGestureRecognizer.handleEvent` and the `PointerPanZoomEndEvent` arm of `stopTrackingIfPointerNoLongerDown` are all absent. | `src/Plumix/UI/PointerEvents.cs`, `src/Plumix/Gestures/GestureRecognizer.cs`, `DragGestureRecognizer.cs`, `src/Plumix/FlutterHost.cs` vs `gestures/events.dart`, `recognizer.dart`, `monodrag.dart` | Port the three event classes (`pan`, `localPan`, `panDelta`, `localPanDelta`, `scale`, `rotation`), add the three recognizer entry points, then have the hosts synthesize them from trackpad input. |
...
| `GestureDetector`/`RawGestureDetector` expose only `onLongPress`/`onLongPressUp`; the recognizer now has the full primary/secondary/tertiary down/cancel/start/moveUpdate/end/up set, reachable only through a `gestures:` factory. | `src/Plumix/Widgets/Gestures.cs` vs `widgets/gesture_detector.dart` | Port the remaining `gesture_detector.dart` long-press parameters and their `_syncAll` wiring; the semantics delegate's long-press handler follows. |
