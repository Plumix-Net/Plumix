...
| `_RenderObjectSemantics` (`RenderObjectSemantics`) has no `debugFillProperties`/`debugDescribeChildren`, so Flutter's `debugDumpSemanticsTree`-style dump of the semantics fragment tree has no counterpart. | `src/Plumix/Rendering/Object.RenderObjectSemantics.cs` vs `rendering/object.dart` (`_RenderObjectSemantics`) | Make it a `DiagnosticableTree` and port the two overrides (owner, `NO PARENT DATA`, `NO GEOMETRY`, `BLOCK PREVIOUS`, `formedSemanticsNode`, sibling groups). |
