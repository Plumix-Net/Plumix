...
| `src/Plumix.Material/Chips.cs` | Flutter's `RawChip` uses a private three-slot render object with independent avatar/checkmark/delete drawers; the current framework lacks slotted render-object widgets, so Action/Choice chip content uses `Row`/`Stack` and only the selection/checkmark transition is animated. | Extreme text scaling and custom avatar constraints can differ by a few pixels; enable/avatar drawer timing is not yet exact, and deletable/input/filter chip slots are not exposed. | Add shared slotted render-object widget support, port `_RenderChip` geometry/animation choreography, and complete the remaining chip family. |
