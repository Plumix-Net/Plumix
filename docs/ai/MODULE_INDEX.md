...
- Desktop text-selection toolbar controls enter through `DesktopTextSelectionToolbar.cs`; delegated viewport
  placement lives in core `Widgets/DesktopTextSelectionToolbarLayoutDelegate.cs`,
  `Widgets/CustomSingleChildLayout.cs`, and `Rendering/CustomSingleChildLayout.cs`, with focused coverage in
  `MaterialDesktopTextSelectionToolbarTests.cs`.
