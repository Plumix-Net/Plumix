...
| Material chips | `src/Plumix.Tests/MaterialChipTests.cs` | `ActionChip`/`ChoiceChip` guards, M2/M3 flat/elevated defaults, theme/widget/state precedence, callbacks, selection/disabled semantics, checkmarks, density, and tap-target geometry. | Specialized `_RenderChip` slot geometry and the full deletable/input/filter chip family. |
