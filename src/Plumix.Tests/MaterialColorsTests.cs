...
public sealed class MaterialColorsTests
{
    ...
    public void MaterialColor_BasicFunctionality()
    {
        ...
        {
            Assert.Equal(new Color((uint)key), color[key]!);
        }

        Assert.Equal(color[50]!, color.Shade50);
        Assert.Equal(color[100]!, color.Shade100);
        Assert.Equal(color[200]!, color.Shade200);
        Assert.Equal(color[300]!, color.Shade300);
        Assert.Equal(color[400]!, color.Shade400);
        Assert.Equal(color[500]!, color.Shade500);
        Assert.Equal(color[600]!, color.Shade600);
        Assert.Equal(color[700]!, color.Shade700);
        Assert.Equal(color[800]!, color.Shade800);
        Assert.Equal(color[900]!, color.Shade900);
    }
    ...
    public void MaterialAccentColor_ExposesItsFourShades()
    {
        ...
        Assert.Equal(accent[100]!, accent.Shade100);
        Assert.Equal(accent[200]!, accent.Shade200);
        Assert.Equal(accent[400]!, accent.Shade400);
        Assert.Equal(accent[700]!, accent.Shade700);
    ...
    public void ColorSwatches_DoNotContainDuplicates()
    {
        foreach (MaterialColor swatch in MaterialColors.Primaries.Append(MaterialColors.Grey))
        {
            Assert.Equal(
                PrimaryKeys.Length,
                PrimaryKeys.Select(key => swatch[key]!).Distinct().Count());
        }
        ...
        foreach (MaterialAccentColor swatch in MaterialColors.Accents)
        {
            Assert.Equal(
                AccentKeys.Length,
                AccentKeys.Select(key => swatch[key]!).Distinct().Count());
        }
    }
    ...
    public void ColorSwatchColors_AreOpaqueAndEqualTheirPrimaryColor()
    {
        foreach (MaterialColor swatch in MaterialColors.Primaries.Append(MaterialColors.Grey))
        {
            Assert.Equal(swatch.Value, swatch.Shade500.Value);
            foreach (int key in PrimaryKeys)
            {
                Assert.Equal(0xFF, swatch[key]!.Alpha);
            }
        }

        foreach (MaterialAccentColor swatch in MaterialColors.Accents)
        {
            Assert.Equal(swatch.Value, swatch.Shade200.Value);
    ...
    public void Palette_MatchesTheDartShapeAndConstants()
    {
        ...
        Assert.Equal(0x00000000u, MaterialColors.Transparent.ToARGB32());
        Assert.Equal(0xFF000000u, MaterialColors.Black.ToARGB32());
        Assert.Equal(0xDD000000u, MaterialColors.Black87.ToARGB32());
        Assert.Equal(0x8A000000u, MaterialColors.Black54.ToARGB32());
        Assert.Equal(0x61000000u, MaterialColors.Black38.ToARGB32());
        Assert.Equal(0xFFFFFFFFu, MaterialColors.White.ToARGB32());
        Assert.Equal(0xB3FFFFFFu, MaterialColors.White70.ToARGB32());
        Assert.Equal(0x99FFFFFFu, MaterialColors.White60.ToARGB32());
        Assert.Equal(0x62FFFFFFu, MaterialColors.White38.ToARGB32());

        Assert.Equal(0xFF2196F3u, MaterialColors.Blue.Value);
        Assert.Equal(0xFFBBDEFBu, MaterialColors.Blue.Shade100.ToARGB32());
        Assert.Equal(0xFF1976D2u, MaterialColors.Blue.Shade700.ToARGB32());
        Assert.Equal(0xFFD32F2Fu, MaterialColors.Red.Shade700.ToARGB32());
        Assert.Equal(0xFF64FFDAu, MaterialColors.TealAccent.Shade200.ToARGB32());

        // `grey` is the only swatch carrying the two extra Material 2 shades.
        Assert.Equal(0xFFD6D6D6u, MaterialColors.Grey[350]!.ToARGB32());
        Assert.Equal(0xFF303030u, MaterialColors.Grey[850]!.ToARGB32());
    ...
    public void ColorSwatch_EqualityIsStructuralOverTheSwatchTable()
    {
        ...
        {
            ["2259 C"] = new Color(0xFF027223),
            ["2273 C"] = new Color(0xFF257226),
        };
        ...
        Assert.Equal(new Color(0xFF027223), first["2259 C"]!);
    ...
    public void ColorSwatch_LerpInterpolatesEveryShadeAndClamps()
    {
        var a = new ColorSwatch<int>(
            0x00000000,
            new Dictionary<int, Color> { [1] = new Color(0x00000000) });
        var b = new ColorSwatch<int>(
            0xFFFFFFFF,
            new Dictionary<int, Color> { [1] = new Color(0xFFFFFFFF) });

        Assert.Equal(0x00000000u, ColorSwatch<int>.Lerp(a, b, 0.0)!.Value);
        // colors_test.dart: `isSameColorAs(const ColorSwatch<int>(0x7F7F7F7F, <int, Color>{1: ...}))`.
        ColorMatchers.AssertSameColorAs(new Color(0x7F7F7F7F), ColorSwatch<int>.Lerp(a, b, 0.5));
        ColorMatchers.AssertSameColorAs(new Color(0x7F7F7F7F), ColorSwatch<int>.Lerp(a, b, 0.5)![1]);
        ...
        ColorMatchers.AssertSameColorAs(new Color(0x7FFFFFFF), ColorSwatch<int>.Lerp(null, b, 0.5));
        ColorMatchers.AssertSameColorAs(new Color(0x7FFFFFFF), ColorSwatch<int>.Lerp(b, null, 0.5));
    }
    ...
    public void ColorScheme_FromSwatchLightDefaults()
    {
        ...
        Assert.Equal(MaterialColors.Blue, scheme.Primary);
        Assert.Equal(MaterialColors.Blue, scheme.Secondary);
    ...
    public void ColorScheme_FromSwatchHonorsEveryOverride()
    {
        ...
        Assert.Equal(MaterialColors.Yellow, scheme.Primary);
    ...
    public void ThemeData_Material2LightDerivesTheSchemeFromTheDefaultSwatch()
    {
        ...
        Assert.Equal(MaterialColors.Blue, theme.ColorScheme.Primary);
        // `accentColor: primarySwatch[500]!`: a plain colour, unlike `primary`, which keeps the swatch.
        Assert.Equal(MaterialColors.Blue.Shade500, theme.ColorScheme.Secondary);
        ...
        Assert.Equal(MaterialColors.Blue, theme.PrimaryColor);
        ...
        Assert.Equal(new Color(0x1F000000), theme.DividerColor);
    ...
    public void ThemeData_Material2DarkDerivesTheSchemeFromTheDefaultSwatch()
    {
        ...
        Assert.Equal(MaterialColors.Grey[850]!, theme.CanvasColor);
        Assert.Equal(theme.CanvasColor, theme.ScaffoldBackgroundColor);
        Assert.Equal(MaterialColors.Grey.Shade800, theme.CardColor);
        Assert.Equal(new Color(0x1FFFFFFF), theme.DividerColor);
        Assert.False(theme.ApplyElevationOverlayColor);
    }
    ...
    public void ThemeData_PrimarySwatchDrivesTheMaterial2Scheme()
    {
        ...
        Assert.Equal(MaterialColors.Green, theme.ColorScheme.Primary);
        // `accentColor: primarySwatch[500]!`: a plain colour, unlike `primary`, which keeps the swatch.
        Assert.Equal(MaterialColors.Green.Shade500, theme.ColorScheme.Secondary);
        Assert.Equal(MaterialColors.Green.Shade200, theme.ColorScheme.Background);
        Assert.Equal(MaterialColors.Green, theme.PrimaryColor);
    ...
    public void ThemeData_OpacityRampColorsIgnoreTheSchemeInBothModes(bool useMaterial3)
    {
        ...
        Assert.Equal(new Color(0x99000000), light.HintColor);
        Assert.Equal(MaterialColors.White60, dark.HintColor);
        Assert.Equal(new Color(0x1F000000), light.FocusColor);
        Assert.Equal(new Color(0x1FFFFFFF), dark.FocusColor);
        Assert.Equal(new Color(0x0A000000), light.HoverColor);
        Assert.Equal(new Color(0x0AFFFFFF), dark.HoverColor);
